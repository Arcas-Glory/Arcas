...
<script lang="ts" setup>
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:5117/api/book/upload', {
        ...
        publishHouse:formInline.publisher,
      ...
      // 处理返回结果
      const vertify = response.data.vertify;
      if (vertify === 'success') {
        ElMessage.success('图书上传成功！')
        setTimeout(()=> {
          router.push({name:'UserDetails',params:{userID:authstore.userID}})
        },100)
      } else {
