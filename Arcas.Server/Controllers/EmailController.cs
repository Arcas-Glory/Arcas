...
namespace Arcas.Server.Controllers
{
    ...
    public class EmailController : Controller
    {
        ...
        [HttpGet("get")]
        public async Task<IActionResult> GetEmails([FromQuery] string token, [FromQuery] int userID = 0)
        {
            // 验证用户名和Token是否有效
            ...
            var emailDTOs = await GetExchangeDetailFromDatabase(userID);
        ...
        private async Task<List<ExchangeDetailDTO>> GetExchangeDetailFromDatabase(int userID)
        {
            ...
            if (user == null)
            {
                return new List<ExchangeDetailDTO>();
            }
            ...
            var emails = await _context.ExchangeDetail
                .Where(e => e.ida == user.userID || e.idb == user.userID)
                .Include(e => e.UserA)  // 加载 UserA
                .Include(e => e.UserB)  // 加载 UserB
                .ToListAsync();

            if (emails == null) return new List<ExchangeDetailDTO>();

            var emailDTOs = emails.Select(e => new ExchangeDetailDTO
            {
                exchangeID = e.exchangeID,
                nickname = e.UserA.nickname,
                IDA = e.ida.ToString(),
                IDB = e.idb.ToString(),
                bookID = e.bookID,
        ...
        public async Task<IActionResult> Apply([FromBody] BookApplyRequest request)
        {
            ...
            // 返回验证结果
            return Ok(new BookApplyResponse { vertify = "success" });
        }
...
public async Task<IActionResult> ReplyToExchangeRequest([FromBody] ReplyExchangeRequest request)
{
    ...
    {
        return BadRequest(new ReplyExchangeResponse{ vertify = "用户无权限回复该申请" });
    }
