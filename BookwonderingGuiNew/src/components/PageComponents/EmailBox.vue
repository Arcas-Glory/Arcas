      ...
      <li 
        ...
        <el-card class="email-item-card" :body-style="{ padding: '10px' }">
          <div class="email-header">
            ...
            <span class="email-time">{{ emailItem.telephone }}</span>
          </div>
...
const username = ref('23454534')
const token = ref('23454')
...
const fetchEmails = () => {
  axios.get('http://localhost:5209/api/Auth/getEmail', {
    params: {
  ...
  .then(response => {
    ...
    emails.value = response.data.emails.map(email => ({
      ...
      telephone: email.telephone || 'None'
    }));
